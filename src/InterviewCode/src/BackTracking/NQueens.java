 ...
 * 
 * 
 * Can be very slow if iterating larger than 13, to minimize time, alter code to return if solution is found
 * 
 */
public class NQueens {
	...
	public static void NQueensHelperExponentialTime(int row, int col, char[][] queensMatrix, int n) {
		int rows = n;
		int cols = n;
		// If we reached the n-th row, or overshot the array b/c 0 based, that
		// means all queens have been placed.
		// Print solution
		if (row == rows) {
			/*
			 * -- Uncomment to print out the solutions
			 * System.out.println("Solution Found: "); for(int i = 0; i < rows;
		...
		// If it is, place it and recurse down to he next row, starting at first
		// column (i + 1, 0)
		// After recursion, backtrack and remove the queen to explore more
		// column positions in this row (i)
	...
	public static boolean isSafeToPlaceQueen(int row, int col, char[][] queensMatrix, int n) {
		int cols = n;
		// Check Row -- Queen will be the only one placed on this row so don't
		// have to handle this case
		// Check Column -- Start from 0 up to the row we are processing only
		// (b/c others not filled yet)
		// Check Diagonal \ and /
